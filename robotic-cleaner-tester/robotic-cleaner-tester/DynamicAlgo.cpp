...
DynamicAlgo::DynamicAlgo(const AbstractSensor& sensor, map<string, int> config)
{
	initiallize();
	setSensor(sensor);
...
DynamicAlgo::DynamicAlgo(void)
{
	initiallize();
}

//function called in each c'tor
void DynamicAlgo::initiallize()
{
	...
	_pathCache = PathCache(vector<GeneralizedPoint>(), 0, 0);
...
//************************************
Direction DynamicAlgo::step(Direction lastStep)
{
	if(_debug) {
...
Direction DynamicAlgo::ChooseStepAccordingToState(Direction lastStep)
{
...
Direction DynamicAlgo::Handle_Returning_State(Direction lastStep){
	if(_debug)	cout << "Handle_Returning_State step number: " << totalSteps << endl;
	int remainingSteps = GetRemainingSteps();
	if(_debug) 
		cout << " DynamicAlgo::Handle_Returning_State\tremainingSteps: " << remainingSteps << endl;
	if(! _pathCache.StillCorrect(lastStep, remainingSteps, AlgorithmStatus::Returning))
	{
		_pathCache = PathCache(GetSPToDocking(), 0 , (size_t)remainingSteps,  AlgorithmStatus::Returning);
...
int DynamicAlgo::GetRemainingSteps()
{
...
Direction DynamicAlgo::Handle_Cleaning_State(Direction lastStep)
{
	if(_debug)	cout << "Handle_Cleaning_State step number: " << totalSteps << endl;
	int remainingSteps = GetRemainingSteps();
	if(_debug) 
		cout << " DynamicAlgo::Handle_Cleaning_State\tremainingSteps: " << remainingSteps << endl;
	if(! _pathCache.StillCorrect(lastStep, remainingSteps, AlgorithmStatus::Cleaning))
	{
		...
		pf.RunIteration();
		_pathCache = PathCache(pf.GetBestPathTo({0,0}), 0 , (size_t)remainingSteps,  AlgorithmStatus::Cleaning);
		if(_debug) cout << "_pathCache was set. size: " << _pathCache.savedPath.size() << " remainingSteps: " << remainingSteps << endl;
...
bool DynamicAlgo::CheckUnexploredPointsReachability()
{
...
Direction DynamicAlgo::Handle_Explore_State()
{
...
int DynamicAlgo::GetPathTotalCleaning(const Path & path) const
{
	int score  = 0;
...
Path DynamicAlgo::GetBestPathToDocking(const deque<Path> & pathsToDocking)
{
...
Path DynamicAlgo::GetSPToDocking()
{
	if(_debug) 
...
deque<Path> DynamicAlgo::GetSPToUnexplored(bool fromDocking /* = false */)
{
	...
	while(! foundUnexplored){
		while( !currentQ.empty() ){
			...
			for ( const auto &dir : houseMapping.at(point).possibleKnownDirections) 
		    {
				...
				if(_debug) cout << "point at " << dir << ": " << neighboor << " distance: " << neighboorDistance << 
					(addToQueue ? " added" : "") << " dirt:" << houseMapping.at(neighboor).dirt << endl;
				if( addToQueue){
					distanceFromP[neighboor] = currDistanceFromP + 1;
					nextQ.push_back(neighboor);
		...
		if(_debug) cout << "currentQ size: " << currentQ.size() << " nextQ size: " << nextQ.size() << endl << endl;
		// switch q's
	...
	currentQ.erase(
		std::remove_if (currentQ.begin(), currentQ.end(), 
			[=](GeneralizedPoint p) -> bool{ return (! houseMapping.at(p).isUnexplored()); }), 
		currentQ.end()
		);
	if(_debug) cout << "currentQ after filter: " << currentQ.size()<< endl;

	deque<Path> shortestPathsToUnexplored;
	for ( const auto &unexploredPoint : currentQ) 
    {
    	Path v;
...
// Returns:   	void
void DynamicAlgo::UpdateState()
{
	// if(_debug){
	// 	cout << "DynamicAlgo::UpdateState" << endl;
...
void DynamicAlgo::aboutToFinish(int stepsTillFinishing)
{
...
void DynamicAlgo::UpdateAlgorithmInfo(Direction lastStep) 
{
	if(_aboutToFinishWasCalled)
		--_stepsTillFinishing;
...
void DynamicAlgo::UpdateNeighboors(SensorInformation s){
	...
	for(Direction d : directions) 
	{
		...
		CellInfo neighboorInfo = CellInfo(-1, -1, false, vector<Direction>(), vector<Direction>());
...
bool DynamicAlgo::CheckAllExplored(){
    for ( const auto &pointInfoPair : houseMapping) 
    {
        if(pointInfoPair.second.isUnexplored())
        	return false;
...
bool DynamicAlgo::CheckAllCleaned(){
    for ( const auto &pointInfoPair : houseMapping) 
    {
...
void DynamicAlgo::UpdateDirt(CellInfo & oldInfo, int newDirtLevel)
{
...
void DynamicAlgo::UpdateInMap(GeneralizedPoint point, CellInfo & newInfo)
{
	...
	if(oldInfo.stepsToDocking > newInfo.stepsToDocking){
		...
		for ( const auto & d : houseMapping.at(point).possibleKnownDirections) 
	    {
	    	GeneralizedPoint neighboor = point;
			neighboor.move(d);
			vector<Direction> neighboorParents = vector<Direction>();
			neighboorParents.push_back(OppositeDirection(d));
			CellInfo neighboorInfo = CellInfo(-1, newInfo.stepsToDocking + 1, false, neighboorParents, vector<Direction>());
...
//for debug:
void DynamicAlgo::printDebugHouseMapping() 
{
...
//for debug
void DynamicAlgo::PrintAlgorithmStatus()
{
...
//REGISTER_ALGORITHM (DynamicAlgo)
