...
Direction _039563838_A::ChooseStepAccordingToState(Direction lastStep)
{
	Direction d;
	switch(_robotStatus)
	{
...
{
	// if(_debug){
	// 	cout << "_039563838_A::UpdateState" << endl;
	...
	switch(_robotStatus)
	{
		...
		case AlgorithmStatus::Exploring:
			{
				...
				else if(houseMapping[position].dirt > 2) // while exploring found dirt and need to stay until cleaned
				{
		...
		case AlgorithmStatus::StayingToClean:
			{
				if(houseMapping[position].dirt <= 2) // no need to stay
				{
